...
# ==============================================================================
# 0. CONFIGURAÇÃO E INFRAESTRUTURA DO BANCO
# ==============================================================================
...
def criar_tabelas():
    ...
    cursor_clinica.execute('''
        CREATE TABLE IF NOT EXISTS Pets (
            ...
            Pet TEXT,
            Especie TEXT,
            Raca TEXT, -- Ajustado de REAL para TEXT
            Sexo TEXT,
    ...
    cursor_clinica.execute('''
        CREATE TABLE IF NOT EXISTS Médicos (
            ...
            CRMV TEXT UNIQUE,
            Email TEXT UNIQUE,
            Turno TEXT, -- Removido UNIQUE para permitir que múltiplos médicos trabalhem no mesmo turno
    ...
    cursor_clinica.execute('''
        CREATE TABLE IF NOT EXISTS Consultas (
            ...
            Diagnostico TEXT, -- Padronizado sem acento para evitar erros de sintaxe
            FOREIGN KEY (Tutor_ID) REFERENCES Tutores(ID),
...
# ==============================================================================
# 1. SETOR: AUTENTICAÇÃO E SEGURANÇA (USUÁRIOS GERAL)
# ==============================================================================
...
# ==============================================================================
# 2. SETOR: PERFIL DO TUTOR E SEUS PETS (ÁREA DO CLIENTE)
# ==============================================================================
...
# ==============================================================================
# 3. SETOR: FLUXO DE AGENDAMENTO DE CONSULTAS (MÉDICOS & CLIENTES)
# ==============================================================================
...
# ==============================================================================
# 4. SETOR: PAINEL DO MÉDICO VETERINÁRIO (ATENDIMENTOS)
# ==============================================================================
...
# ==============================================================================
# 5. SETOR: GESTÃO DO ADMINISTRADOR (CADASTROS & TRIAGEM OPERACIONAL)
# ==============================================================================
...
def listar_consultas_geral():
    ...
    cursor_clinica.execute('''
        SELECT 
            C.ID, T.Nome, P.Pet, M.Nome, C.Data, C.Horario, C.Status, C.Diagnostico
        FROM Consultas C
        JOIN Tutores T ON C.Tutor_ID = T.ID
        JOIN Pets P ON C.Pet_ID = P.ID
        LEFT JOIN Médicos M ON C.Medico_ID = M.ID
        WHERE C.Status != 'Finalizado pelo Admin'
    ''')
    consultas = cursor_clinica.fetchall()
    cursor_clinica.close()
    conexao_clinica.close()
