...
def criar_tabelas():
    ...
    cursor_pagamento.execute('''
        CREATE TABLE IF NOT EXISTS Pets (
            ...
            FOREIGN KEY (Nome_Tutor) REFERENCES Tutores(Nome)
        )
    ''')
...
conexao_pagamento.execute('''
                          INSERT INTO Cartao (Usuario_ID, Nome, Email, Data_Validade, CVV, Bandeira, Valor, Data_Hora, Status)
                          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                          ''', (st.session_state.usuario_id, f"****{numero[-4:]}", nome, email, crmv))
    ...
    with col1:
        ...
        if st.button("Confirmar"):
            if not email or not senha:
                ...
                conexao_pagamento = conectar_banco()
                cursor_pagamento = conexao_pagamento.cursor()
                ...
                if not email_valido:
                    ...
                    if modo == "Login":
                        cursor_pagamento.execute("SELECT ID FROM Usuarios WHERE Email = ? AND Senha = ?", (email, senha_hash))
                        usuario = cursor_pagamento.fetchone()
                        if usuario:
                        ...
                        cursor_pagamento.execute("SELECT * FROM Usuarios WHERE Email = ?", (email,))
                        if cursor_pagamento.fetchone():
                            st.warning("E-mail já cadastrado.")
                        else:
                            cursor_pagamento.execute("INSERT INTO Usuarios (Email, Senha) VALUES (?, ?)", (email, senha_hash))
